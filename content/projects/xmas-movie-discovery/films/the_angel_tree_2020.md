+++
title = """The Angel Tree"""
template = "xmas-movie-page.html"
description = """A writer seeks the identity of the person who helps grant wishes that are placed upon the angel tree, and in the process, reconnects with her childhood friend."""

[taxonomies]
xmas_genres = ['Mystery', 'Hallmark', 'TV Movie', 'Romance']

[extra]
film_url = """https://letterboxd.com/film/the-angel-tree/"""
tmdb_poster_path = """/3ykv13npXiwTQiMv3Qf2VXIOdPW.jpg"""
year = 2020
director = """Jessica Harmon"""
runtime = 84
countries = ["""Canada""", """USA"""]
original_language = """English"""
spoken_languages = ["""English""", """Spanish"""]
average_rating = 3.11
cast = ["""Jill Wagner""", """Lucas Bryant""", """Cassidy Nugent""", """Clare Filipow""", """Oscar Farrell""", """Tom Pickett""", """Annabel Kershaw""", """Jason Burkart""", """Jayce Barreiro""", """Vera Ann Frederickson""", """Shay Galor""", """Hayden Carmichael""", """Ethel Pitchford""", """Tyson Larter""", """Isabella Sleeth""", """Candus Churchill""", """Kalyn Miles"""]
+++
