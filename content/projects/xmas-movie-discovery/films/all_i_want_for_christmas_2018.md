+++
title = """All I Want for Christmas"""
template = "xmas-movie-page.html"
description = """To be the first girl accepted into Santa school, twelve-year-old Lucia must find a cure for a mystery illness caused by the devilish Krampus."""

[taxonomies]
xmas_genres = ['Family', 'Global', 'Adventure']

[extra]
film_url = """https://letterboxd.com/film/all-i-want-for-christmas-2018/"""
tmdb_poster_path = """/ouptYjB32YEhRdrvRW962xaSLsE.jpg"""
year = 2018
director = """Christian Dyekjær"""
runtime = 92
countries = ["""Denmark"""]
original_language = """Danish"""
spoken_languages = ["""Danish"""]
average_rating = 3.08
cast = ["""Martin Buch""", """Mia Lyhne""", """Mette Agnete Horn""", """Nicolaj Kopernikus""", """Ulf Pilgaard""", """Kristian Halken""", """Ella Testa Kusk""", """Peter Sejer Winther""", """Anders Brink Madsen""", """Peter Gantzler""", """Julie Agnete Vang""", """Mette Lysdahl""", """Kristian Fjord"""]
+++
